...
function bp_widget_cache_display( $instance, $widget_object, $args ) {
	...
	/**
	 * Add filter before outputting widget cache.
	 *
	 * @param string $output        Widget contents to display.
	 * @param object $widget_object Current widget class object.
	 * @param array  $instance      Current widget arguments.
	 * @param array  $args          Widget sidebar arguments.
	 */
	$output = apply_filters( 'bp_widget_cache_output', $cache[$instance_key], $widget_object, $instance, $args );
...
/**
 * Replace nonces in certain widgets.
 *
 * Some widgets use nonces. These items need to be generated at run-time
 ...
 * @return string
 */
function bp_widget_cache_output_replace_nonces( $retval, $widget ) {
	// Nonce replace function.
	$replacer = function( $contents, $action, $name ) {
		$nonce_pos     = strpos( $contents, sprintf( '<input type="hidden" id="%1$s" name="%2$s"', $name, $name ) );
		$nonce_end_pos = strpos( $contents, ' />', $nonce_pos + 1 );

		$nonce     = substr( $contents, $nonce_pos, $nonce_end_pos + 3 - $nonce_pos );
		$new_nonce = wp_nonce_field( $action, $name, false, false );

		// Replace the old nonce with new one.
		return substr_replace( $contents, $new_nonce, $nonce_pos, strlen( $nonce ) );
	};

	// Groups widget.
	if ( $widget instanceof BP_Groups_Widget ) {
		$retval = $replacer( $retval, 'groups_widget_groups_list', '_wpnonce-groups' );
	}

	// Members widget.
	if ( $widget instanceof BP_Core_Members_Widget || $widget instanceof CAC_Members_Widget ) {
		$retval = $replacer( $retval, 'bp_core_widget_members', '_wpnonce-members' );
	}

	// Friends widget.
	if ( $widget instanceof BP_Core_Friends_Widget ) {
		$retval = $replacer( $retval, 'bp_core_widget_friends', '_wpnonce-friends' );
	}

	return $retval;
}
add_filter( 'bp_widget_cache_output', 'bp_widget_cache_output_replace_nonces', 10, 2 );
